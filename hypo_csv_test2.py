...
def kalman_update(track, P, measurement, R):
    ...
    # Updated State: track_updated = track + K (measurement - track)
    track_updated = track + K @ (measurement - track)
    
    # Updated Covariance: P_updated = (I - K) P
    P_updated = (np.eye(state_dim) - K) @ P
    
    return track_updated, P_updated

# Write hypotheses to CSV
with open('hypotheses2.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Tracks/Reports", "Hypothesis", "Probability", "Joint Probability"])
    ...
    for track_idxs, report_idxs in clusters:
        ...
        writer.writerow([f"Tracks/Reports: {['t' + str(i+1) for i in track_idxs]}"])
        for hypothesis, prob, joint_prob in zip(hypotheses, probabilities, joint_probabilities):
            ...
            writer.writerow([None, f"Hypothesis: {formatted_hypothesis}", f"Probability: {prob:.4f}", f"Joint Probability: {joint_prob:.4f}"])
